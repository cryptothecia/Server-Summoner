...
PATHSfile = os.path.join(parent_path,".PATHS")
external_ip = None
games = []
bot_host = ''
host = ''
...
def get_bot_host():
    global bot_host
    bot_host = read_PATHS("botHost=")
    if bot_host != '':
        bot_host = socket.gethostbyname(bot_host)

# Build list of games that the dedicated server can run
def get_games():
    global games
    games = glob.glob(service_path)
    i = 0
    for game in games:
        games[i] = (
            game.replace((service_path.split("*"))[0],'')
            .replace((service_path.split("*"))[1],'')
        )
        i+=1

# Creates new Server, ServerStop and Backup .services for the requested game
def create_game_services(game:str):
    path = service_path.replace('*',game)
    with open(os.path.join(parent_path,"TEMPLATE_Server.service"),"r") as f:
        server_service=(
            f.read()
            .replace("Game",game)
            .replace("PATH/",str(parent_path))
        )
    with open(os.path.join(parent_path,"TEMPLATE_Backup.service"),"r") as f:
        backup_service=(
            f.read()
            .replace("Game",game)
            .replace("PATH/",str(parent_path))
    ...
    with open(os.path.join(parent_path,
                           "TEMPLATE_ServerStop.service"),"r") as f:
        server_stop_service=(
            f.read()
            .replace("Game",game)
            .replace("PATH/",str(parent_path))
        )
    def create_service(service_path, service_body):
        with open(service_path,"w") as f:
            f.write(service_body)
        if (os.path.isfile(service_path)) is True:
            subprocess.run(["sudo", "chmod", "+x", service_path], text=True)
    create_service(path,server_service)
...
def check_game_services(game:str,backup_requested:bool):
    path = service_path.replace('*',game)
    if (os.path.isfile(path)) is False:
        create_game_services(game)
    server_status = subprocess.run(['systemctl', 
                                   'is-active', 
                                   f'{game}Server.service'], 
                                   text=True, 
                                   stdout=subprocess.PIPE)
    if backup_requested == True:
        backupStatus = subprocess.run(['systemctl', 
                                       'is-active', 
                                       f'{game}Backup.service'], 
                                       text=True, 
                                       stdout=subprocess.PIPE)
        return server_status.stdout.strip(), backupStatus.stdout.strip()
    else: 
        return server_status.stdout.strip()
 
# Starts the services for the requested game 
def start_game_services(game:str,backup_requested:bool):
    subprocess.run(['sudo', 
                    'systemctl', 
                    'start', 
                    f'{game}Server.service'], 
...
def get_current_game():
    ag = []
    for game in games:
        server_status = check_game_services(game,False)
...
def reply(request):
    ...
    match request:
        case "status":
            active_games = get_current_game()
            if active_games == None: 
                return " running"
            else: 
                r = ''.join(active_games)
                if r in games:
        ...
        case request if request != "status" and request != "shutdown":
            server_status, backupStatus = check_game_services(request,True)
            if server_status == "inactive":
                ...
                if active_games is None:
                    ...
                    if backupStatus == "inactive":
                        backup_request = True
                    start_game_services(request,backup_request)
                    return (f"Bringing {request} server online.::"
                            f"{external_ip}::"
                            f"{read_PATHS(f"_{request}_Port=")}")
                else:
                    return ''.join(active_games) + f" running::{external_ip}"
...
def main():
    ...
    while True:
        ...
        if request: 
            ...
            # Checks addr IP against the IP of bot_host, 
            # defined by get_bot_host(). 
            # If IP is not the same, no action should be taken 
            # and a reject message sent. 
            # This check doesn't happen if bot_host ends up blank
            if bot_host != '' and addr[0] == bot_host:
                answer = reply(request)
            elif bot_host != '' and addr[0] != bot_host:
